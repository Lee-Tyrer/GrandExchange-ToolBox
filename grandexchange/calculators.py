...
# def average_time_in_exchange(prices: list[Price], volume: int, timestep: int = None) -> float:
#     """Calculates the average time the item remains in the grand exchange
#
#     Uses the historical values to determine the volume of sales in each
#     time gap. The aim of this function was to calculate how quickly the turn
#     around time of a flip will be. Ideally, items that are quickly sold
#     with moderate margins can instantly boost your profits.
#
#     Parameters
#     ----------
#     prices: list[Price]
...
#         price = prices[0]
#         try:
#             rate = price.volume / (60 / timestep)
#         except TypeError:
#             raise TypeError("Timestep must be provided and can not be None")
#         return volume * rate
#
#     for index, p in enumerate(prices):
#         if index - 1 >= 0:
#             diff = p.timestamp - prices[index - 1].timestamp
#             rates.append(p.volume / diff)
#
#     return volume / mean(rates)
#
#
# def best_profit_efficiency(timeseries: list[Timeseries]):
#     sales = []
#     for ts in timeseries:
#         latest = ts.latest_offer()
#         try:
#             transaction = flip(latest, volume=1)
#         except PriceNotAvailableError:
#             continue
#
#         if ts.item.limit is None:
#             continue
#
#         potential_profit = transaction.profit * ts.item.limit
#
#         # Time in hours
#         time_in_exchange = average_time_in_exchange(ts.lowest, ts.item.limit, ts.timestep)
#
#         latest.attributes["profit efficiency"] = potential_profit * time_in_exchange
#
#         sales.append(latest)
#
#     return sales
